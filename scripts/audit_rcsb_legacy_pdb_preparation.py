#!/usr/bin/env python3
"""Audit strict direct Meeko preparation of an RCSB legacy-PDB source set."""
...
def classify(log: str) -> str:
    value = log.lower()
    if "alternate location" in value:
        return "alternate_locations"
    if "no template matched" in value or "template matching failed" in value:
...
def main() -> None:
    ...
    fields = ["target", "pdb_id", "pdb_sha256", "outcome", "return_code", "pdbqt_bytes", "error_class"]
    rows = []
    def write_rows() -> None:
    ...
    for record in records:
        ...
        source = args.pdb_dir / f"{pdb_id}.pdb"
        common = {"target": target, "pdb_id": pdb_id, "pdb_sha256": "", "outcome": "", "return_code": "", "pdbqt_bytes": 0, "error_class": ""}
        if not source.exists():
            rows.append({**common, "outcome": "not_evaluated", "error_class": "source_missing"})
        else:
            common["pdb_sha256"] = sha256(source)
            output_dir = args.work_dir / target
            output_dir.mkdir(parents=True, exist_ok=True)
            output_base = output_dir / "receptor"
            pdbqt = Path(f"{output_base}.pdbqt")
            try:
                run = subprocess.run(
                    [args.meeko_command, "--read_pdb", str(source), "--output_basename", str(output_base), "--write_pdbqt"],
                    capture_output=True, text=True, timeout=args.timeout_seconds, check=False,
                )
                log = run.stdout + run.stderr
                (output_dir / "prepare_receptor.log").write_text(log, encoding="utf-8")
